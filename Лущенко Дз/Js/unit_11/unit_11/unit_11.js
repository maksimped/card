...
function f15() {
    ...
    for (let i = 0; i < d15.length; i++) {
        if (+input15.value == d15[i]) {
            isTrue = true;

        }
    }
    if (isTrue == false) {
        d15.push(+input15.value)
    }
...
function f18() {
    ...
    let isInclude = d18.includes(input18.value)
    document.querySelector('.out-18').innerHTML = isInclude
}
...
function f19() {
    let isString = d19[0].length;
    for (let i = 0; i < d19.length; i++) {
        if (d19[i].length > isString) {
            isString = d19[i].length;
            maxString = d19[i];
        }
    }
